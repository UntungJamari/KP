...
<table id="table1" data-cols-width="4,14,12,12,30,20,10,10,14,14,20,20,20" style="color: white;">
    ...
    <tr>
        ...
        $query = mysqli_query($koneksi, "select * from kemenag_kab_kota, kab_kota where kab_kota.id_kab_kota=kemenag_kab_kota.id_kab_kota and kemenag_kab_kota.username='$username'");
        $result = mysqli_fetch_assoc($query);

        ?>
        <th data-a-h="center" colspan="13">Kementerian Agama <?php echo $result['nama_kab_kota']; ?></th>
    </tr>
    <tr>
    ...
    $query = mysqli_query($koneksi, "select * from pengawasan, ppiu, kemenag_kab_kota where pengawasan.id_ppiu=ppiu.id_ppiu and ppiu.id_kab_kota=kemenag_kab_kota.id_kab_kota and kemenag_kab_kota.username='$username'");
    $no = 1;
...
<script>
    ...
    TableToExcel.convert(document.getElementById("table1"), {
        name: "Blanko Pengawasan Umrah Kemanag " + "<?php echo $result['nama_kab_kota']; ?>" + ".xlsx",
        sheet: {
            name: "Sheet 1"
        }
    });
